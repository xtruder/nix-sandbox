...
let
  ...
  fontsConfig = pkgs.writeText "fonts.conf" ''
  ...
  -->
    <match target="pattern">
      <test qual="any" name="family" target="default" ignore-blanks="false" compare="eq">
        <string>mono</string>
      </test>
  ...
  <dir>${pkgs.xorg.fontbhttf}/lib/X11/fonts</dir>
  <dir>${pkgs.ttf_bitstream_vera}/share/fonts</dir>
  </fontconfig>
  '';
  ...
  xorgConfig = pkgs.writeText "xorg.conf" ''
    ...
    Section "Files"
      ...
      ModulePath "${pkgs.xorg.xf86videointel}/lib"
      ModulePath "${pkgs.xorg.xf86videointel}/lib/xorg/modules/drivers"
      ...
      ModulePath "${pkgs.xorg.xf86inputsynaptics}/lib/xorg/modules/input"
    EndSection
  ...
  ldSoPreload = pkgs.writeText "ld.so.preload" ''
    ${pkgs.virtualglLib}/lib/libdlfaker.so ${pkgs.virtualglLib}/lib/libvglfaker.so ${pkgs.virtualglLib}/lib/libvglfaker-nodl.so ${pkgs.virtualglLib}/lib/libgefaker.so
  '';

  gtkrcConfig = pkgs.writeText "gtkrc" ''
  ...
  X11Opts = {
    storePaths = [pkgs.fontconfig.out fontsConfig];
    mounts = [
      "${fontsConfig}:/etc/fonts/fonts.conf"
      "${pkgs.fontconfig.out}/etc/fonts/conf.d:/etc/fonts/conf.d"
      "${gtkrcConfig}:/home/${user}/.gtkrc-2.0"
      "${gtkrcConfig}:/home/${user}/.gtkrc-3.0"
      # "${pkgs.gnome3.adwaita-icon-theme}/share/icons:/home/${user}/.local/share/icons"
      # "${pkgs.gnome3.gnome_themes_standard}/share/themes:/home/${user}/.themes"
    ];
    env = {
      DISPLAY=":10";
      ...
      XDG_DATA_DIRS="${pkgs.gnome3.gnome_themes_standard}/share:${pkgs.gnome3.adwaita-icon-theme}/share";
      GTK_PATH="${pkgs.gnome3.gtk.out}/lib/gtk-3.0:${pkgs.gnome.gtk.out}/lib/gtk-2.0";
      GTK_DATA_PREFIX="${pkgs.gnome3.gtk.out}";
      /*LD_LIBRARY_PATH="/run/opengl-driver/lib";*/
    };
  };

  browserBin = pkgs.writeScript "start-browser.sh" ''
      echo "@@@@@@@@@@@@@@@@@@@@@@@@@@1"
      vglrun -d :10 glxgears
      echo "@@@@@@@@@@@@@@@@@@@@@@@@@@2"
      exit 1

    chromiumExitCode="0"
    while [ "$chromiumExitCode" == "0" ]; do
      vglrun -d :10 chromium
      chromiumExitCode="$?"
    done
  '';
  xvfbBin = pkgs.writeScript "start-xvfb.sh" ''
    #!${pkgs.busybox}/bin/sh
    xpra_Xdummy -xkbdir ${pkgs.xkeyboard_config}/etc/X11/xkb -noreset -nolisten tcp +extension GLX +extension RANDR +extension RENDER -auth /tmp/.Xauthority -logfile /tmp/Xorg.log -configdir /tmp/xorg.conf.d -config ${xorgConfig} :10
         #Xvfb -xkbdir ${pkgs.xkeyboard_config}/etc/X11/xkb -noreset -nolisten tcp +extension GLX +extension RANDR +extension RENDER -auth /tmp/.Xauthority -logfile /tmp/Xorg.log -configdir /tmp/xorg.conf.d -config ${xorgConfig} :10
  '';
  startBin = pkgs.writeScript "start-xpra.sh" ''
    rm -rf $HOME/.xpra/
    rm -f /tmp/.X10-lock
    #xpra start :10 --opengl=yes --clipboard=GDK --speaker=on --exit-with-children --xvfb=${xvfbBin} --start-child=${browserBin} --daemon=off --bind-tcp=0.0.0.0:10000 --html=0.0.0.0:8080 --no-mdns --no-notifications
    xpra start :10 --opengl=yes --clipboard=GDK --speaker=on --exit-with-children --xvfb=${xvfbBin} --start-child=${browserBin} --daemon=off --bind-tcp=0.0.0.0:10000 --html=0.0.0.0:8080 --no-mdns --no-notifications
    cat /tmp/Xorg.log

  '';
  PYTHONPATH = lib.makeSearchPath "lib/${pkgs.pythonPackages.python.libPrefix}/site-packages" [
    pkgs.pythonPackages.netifaces
    pkgs.pythonPackages.numpy
    pkgs.pythonPackages.websockify
    pkgs.pythonPackages.pygobject3.out
    pkgs.pythonPackages.gst-python.out
  ];
...
in {
  name = "someenv";
  preStart = ''
    mkdir -p $HOME/data/${config.name}
    ${concatStrings (mapAttrsToList (n: v: ''
      mkdir -p $HOME/data/${config.name}/${v.name}
    '') config.containers)}
  '';

  containers.chromium = mkMerge [defaultOpts X11Opts {
    packages = [
      pkgs.xpra startBin pkgs.busybox pkgs.xorg.xorgserver.out
      xorgConfig pkgs.xorg.xf86videodummy pkgs.xorg.xf86inputevdev
      pkgs.xorg.xf86inputsynaptics pkgs.xkeyboard_config
      pkgs.mesa_drivers pkgs.mesa_glu pkgs.pythonPackages.websockify
      pkgs.gnome3.adwaita-icon-theme pkgs.gnome3.gnome_themes_standard
      pkgs.pythonPackages.netifaces pkgs.pythonPackages.numpy
      pkgs.pythonPackages.pygobject3.out
      pkgs.gst_all_1.gst-plugins-base
      pkgs.gst_all_1.gst-plugins-good
      pkgs.gst_all_1.gst-plugins-bad
      ...
      pkgs.pythonPackages.gst-python
      pkgs.pulseaudioLight.out
      pkgs.vlc
      pkgs.chromium pkgs.fontconfig.bin pkgs.xorg.fontbhttf
      pkgs.ttf_bitstream_vera
      pkgs.gnome.gtk.out pkgs.gnome3.gtk.out pkgs.virtualgl pkgs.virtualglLib
      pkgs.glxinfo ldSoPreload
    ];
    command = "sh ${startBin}";
    ports = ["10000:10000" "8080:8080"];
    interactive = true;
    tty = true;
    /*mounts = [ "${pkgs.mesa_noglu.drivers}:/run/opengl-driver" ];*/
    mounts = [
        /*"/run/opengl-driver/lib/dri/swrast_dri.so:/run/opengl-driver/lib/dri/swrast_dri.so"*/
        "${pkgs.mesa_drivers}/lib:/run/opengl-driver/lib"
